...
if (enabled && (!appId || !appSecret)) throw new Error("PRIVY_APP_ID and PRIVY_APP_SECRET are required when PRIVY_AUTH_ENABLED=true");
...
export async function authenticateCompany(req: IncomingMessage, store: JsonFileStore, verify: (token: string) => Promise<{ userId: string }> = async token => {
  ...
  if (!enabled) throw new Error("company_auth_disabled"); return privy!.verifyAuthToken(token);
