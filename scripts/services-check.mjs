...
if (process.env.PRIVY_AUTH_ENABLED !== "true") throw new Error("PRIVY_AUTH_ENABLED must be true");
...
const factoryArtifact = JSON.parse(await readFile("artifacts/contracts/ChallengeFactory.json", "utf8"));
...
const factoryBytecode = await client.getBytecode({ address: process.env.CHALLENGE_FACTORY_ADDRESS });
if (!factoryBytecode) throw new Error("factory bytecode missing");
if (factoryBytecode.toLowerCase() !== factoryArtifact.deployedBytecode.toLowerCase()) throw new Error("configured factory bytecode does not match generated production artifact");
